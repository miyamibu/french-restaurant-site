 ...
 * ```
 */
const Button = forwardRef<HTMLButtonElement, LkButtonProps>(function Button(
  {
    label = "Button",
    variant = "fill",
    color = "primary",
  ...
  // Define local style fallbacks so tokens work without importing all LiftKit utility CSS files.
  const baseButtonStyle =
    variant === "fill"
      ? {
          backgroundColor: `var(--lk-${color})`,
          color: `var(--lk-${onColorToken})`,
        }
      : {
          color: `var(--lk-${color})`,
        };

  const baseButtonClasses = modifiers ?? "";
